...
import { ListarTodosComponent } from '../listar-todos';
import { CriarOuEditarTodoPayload, Todo, TodoService } from 'src/app/core/services/todo/todo.service';
...
export class ModalCriarEditarTodoComponent {
  ...
  private obterPayloadCriarOuEditarTodo(): CriarOuEditarTodoPayload {
  ...
  private editarTodo() {
    ...
    this.httpClient.patch(`http://localhost:8000/todo/${this.todo?.id}/`, body).subscribe(()=> {
      this.matDialogRef.close({
        recarregar: true
      })
  }).add(() => {
